...
public class AdbMessage {
    ...
    public void set(int command, int arg0, int arg1, byte[] data) {
        ...
        switch (command){
            case A_WRTE:
                //L.i("ADB流程 发送的指令：A_WRTE arg0:"+arg0+" arg1:"+arg1+" datalength:"+(data == null ? 0 : data.length));
                break;
            case A_CLSE:
                //L.i("ADB流程 发送的指令：A_CLSE arg0:"+arg0+" arg1:"+arg1+" datalength:"+(data == null ? 0 : data.length));
                break;
            case A_OKAY:
                //L.i("ADB流程 发送的指令：A_OKAY arg0:"+arg0+" arg1:"+arg1+" datalength:"+(data == null ? 0 : data.length));
                break;
            case A_OPEN:
                //L.i("ADB流程 发送的指令：A_OPEN arg0:"+arg0+" arg1:"+arg1+" datalength:"+(data == null ? 0 : data.length));
                break;
            case A_CNXN:
                //L.i("ADB流程 发送的指令：A_CNXN arg0:"+arg0+" arg1:"+arg1+" datalength:"+(data == null ? 0 : data.length));
                break;
            case A_SYNC:
    ...
    public String getDataString() {
        ...
        // trim trailing zero
        return new String(mDataBuffer.array(), 0, length);
    }
            ...
            if (request.queue(mMessageBuffer, 24)) {
                ...
                if (length > 0) {
                    ...
                    if (request.queue(mDataBuffer, length)) {
                        //L.i("mDataBuffer = "+ Arrays.toString(mDataBuffer.array()));
                        return true;
                    } else {
